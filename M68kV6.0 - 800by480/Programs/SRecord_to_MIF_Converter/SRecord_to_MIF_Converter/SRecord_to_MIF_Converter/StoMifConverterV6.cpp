...
int main(int argc, char *argv[])
{
  ...
  if((SourceFilePtr =  fopen("C:\\M68kV6.0 - 800by480\\Assignment6\\PARTB_PROJ\\partb.hex", "r")) == NULL)	{
		fprintf(stderr, "Cannot Open S-Record File '.hex' for conversion to '.MIF' format\n") ;
		fprintf(stderr, "Hit any key to exit.....") ;
		getch() ;
		return 0 ;
	}
	
	if((DestFilePtr = fopen("C:\\M68kV6.0 - 800by480\\Assignment6\\PARTB_PROJ\\partb.mif","w")) == NULL)	{
		fprintf(stderr, "Cannot Open/Create file '.mif' to store output of file conversion\n") ;
		fprintf(stderr, "Hit any key to exit.....") ;
		getch() ;
		return 0 ;
	}  
	
// write this to the output file, just for compatibilitys

 ...
 * www.netlib.org benchmark/whetstone version obtained 16 March 1998.
 *
 ...
 *
 * Define PRINTOUT to cause the POUT() function to print outputs at
 * various stages.  Final timing measurements should be made with the
 * PRINTOUT undefined.
 *
...
C
C     o	This is a REAL*8 version of the Whetstone benchmark program.
C
...
int main()
{
	...
	printf("Now running Whetstone benchmark ... \n");
...
*/
	...
	printf("C Converted Double Precision Whetstones: %d\n", LOOP * II);

	kips = (100.0 * LOOP * II)/(float)(finisec - startsec);

	if (kips >= 1000)
		printf("This (virtual) machine benchmarks at %.0lf MIPS\n", kips / 1000.0);
	else
		printf("This (virtual) machine benchmarks at %.0lf KIPS\n", kips);
...
void POUT(long N, long J, long K, double X1, double X2, double X3, double X4)
{
	printf("%7ld %7ld %7ld %12.4le %12.4le %12.4le %12.4le\n", N, J, K, X1, X2, X3, X4);
}
#endif